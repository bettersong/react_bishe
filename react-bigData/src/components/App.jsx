...
import bigHouseData from './pages/bigHouseData'
...
class App extends Component {
  ...
  getLinkData = async () => {
    ...
    if(res.status===200){
      let arr = res.data.filter(item=>{return item.read_tag==1})
      console.log(arr,666)
        this.setState({
            msgTotal: arr.length
        })
  ...
  render() {
    const { collapsed,colorList,defaultColor,userInfo,msgTotal} = this.state
      ...
      <Layout ref="layout" style={{ height: '100%', background: `url(${defaultColor}) 0% 0% / cover`}}>
        ...
        <Sider trigger={null} collapsible collapsed={collapsed}>
          ...
          <Menu theme="dark" mode="inline" defaultSelectedKeys={[window.location.pathname]} defaultOpenKeys={[window.location.pathname.split('/')[1]]}>
            ...
            <SubMenu key="big_house_data" title={<Fragment><Icon type="link" /><span>系统数据</span></Fragment>}>
              <Menu.Item key="/big_house_data/index">
                <NavLink to="/big_house_data/index">大棚数据</NavLink>
              </Menu.Item>
            </SubMenu>
            {content2}
        ...
        <Layout style={{ background: `url(${defaultColor}) 0% 0% / cover`}}>
          <Header style={{ padding: 0,borderBottom:'1px solid rgba(255,255,255,0.2)' }}>
            ...
            <div className="user">
              <span style={{ marginRight: 8 }}>
                <Badge count={msgTotal}><Avatar src={userInfo.photo} /></Badge>
              </span>
              <Dropdown overlay={
                <Menu>
                  <Menu.Item key="1">
          ...
          >
              <Switch>
                ...
                <Route path="/big_house_data/index" component={bigHouseData} />
                ...
                <Route path="/solutionSearch" component={SolutionSearch} getLinkData={this.getLinkData} />
